...
void DrawVintv2_STAR_BES(){
    ...
    Float_t AxisXmin = -0.09;
    Float_t AxisXmax = 3.99;
    Float_t AxisYmin = -0.49;
    Float_t AxisYmax = 4.99;
    ...
    //         drawOdj.back()->NCQUndo(nq[pid],mass[pid]); // un-NCQ it
    ...
    canvas->SetTextInfo(Form("#font[42]{#scale[1.0]{ v_{2}/v_{2}^{int} }}"),"Y", 1,0.7,0.55,0.5,90.0);
    canvas->SetTextInfo(Form("#font[42]{#scale[1.0]{ p_{T} (GeV/c^{2}) }}"),"X", 1,0.3,0.5,0.5,0.0);
    ...
    result->SaveAs(Form("./Picture_Vintv2_BES.png"));
    result->SaveAs(Form("./Picture_Vintv2_BES.pdf"));
